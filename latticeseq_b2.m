function x = latticeseq_b2(s, n, N)
...
% Usage:
%
%   1. Initialize the generator with the default lattice sequence (see below)
%
%     latticeseq_b2 init0
%
%   or initialize the generator with a user supplied generating vector for n
%   points (n is optional and defaults to 2^32)
%
%     latticeseq_b2('init0', zvec, n)
%
...
%     latticeseq_b2 state
%
% The default lattice sequence is the ``universal'' lattice sequence optimized
% for an unanchored Sobolev space with order-2 weights from the paper by Cools,
% Kuo and Nuyens in SIAM SISC (2006).  This sequence is ``universal'' in the
% sense that the weights do not really matter, as such, there are no good or
% bad weights...  For this lattice sequence the maximum number of dimensions is
% 250 and the maximum number of points is 2^20 = 1048576. The worst-case error
% was optimized starting from 2^10 = 1024 points (this means it may perform
% badly for less than 1024 samples).
%
%
% Example:
%   latticeseq_b2('init0');        % Initialize the generator with the default rule.
...
%
% (C) 2005, 2010, Dirk Nuyens, Department of Computer Science, KU Leuven, Belgium
%     2016 swap initialization order of n and zvec
n=10
N=1024
persistent z smax k Nmax b m initmode recipd maxbit
...
if ischar(s) && strncmp(s, 'init', 4)
    ...
    if (nargin > 1)
        ...
        z = n; % the second argument is the generating vector
        if (nargin == 3) && ~isscalar(N)
            error('Initialization has the form initmode[, z[, N]], your N is not a scalar')
        elseif (nargin == 3) 
            Nmax = N;
        else    
            Nmax = pow2(32);
...
if ((k + n) > Nmax) || (s > smax)
    error('Can only generate %d lattice points in %d dimensions', Nmax, smax);
end
